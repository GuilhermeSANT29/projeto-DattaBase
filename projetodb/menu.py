...
# --- CONFIGURE AQUI (já com suas credenciais) ---
DB_HOST = "localhost"
DB_USER = "ADM"
...
def registrar_leitura():
    ...
    try:
        ...
        for m in maquinas:
            print(f"{m[0]} - {m[1]}")
        maquina_id = input("ID da máquina: ").strip()
...
def cadastrar_alerta():
    ...
    try:
        ...
        maquina_id = None
        if maquinas:
            for m in maquinas: print(f"{m[0]} - {m[1]}")
        ...
        sensor_id = None
        if sensores:
            for s in sensores: print(f"{s[0]} - {s[1]}")
...
def cadastrar_manutencao():
    ...
    try:
        ...
        operador_id = None
        if operadores:
            for o in operadores: print(f"{o[0]} - {o[1]}")
    ...
    while True:
        ...
        try:
            if op == "1":
                ...
                cur.execute("""
                    SELECT l.id, m.nome, s.nome, o.nome, l.valor, l.momento
                    FROM leituras l
                ...
                cur.execute("""
                    SELECT a.id, m.nome, s.nome, a.nivel, a.descricao, a.momento
                    FROM alertas a
                ...
                cur.execute("""
                    SELECT mt.id, m.nome, o.nome, mt.descricao, mt.data_prevista, mt.concluido, mt.momento
                    FROM manutencoes mt
